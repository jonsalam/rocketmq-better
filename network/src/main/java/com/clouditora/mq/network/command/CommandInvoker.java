    ...
    public Command syncInvoke(Channel channel, Command request, long timeout) throws InterruptedException, SendException, TimeoutException {
        ...
        try {
            ...
            if (commandFuture.isSendOk()) {
                throw new TimeoutException(NetworkUtil.toEndpoint(channel), timeout, commandFuture.getCause());
            } else {
                throw new SendException(NetworkUtil.toEndpoint(channel), commandFuture.getCause());
            }
        } finally {
            ...
            channel.writeAndFlush(request)
                    .addListener((ChannelFutureListener) f -> {
                        ...
                        if (prev != null) {
                            ...
                            log.warn("send request command failed: channel={}", NetworkUtil.toEndpoint(channel));
                            try {
                                NetworkUtil.invokeCallback(prev, getCallbackExecutor());
                            } catch (Throwable e) {
