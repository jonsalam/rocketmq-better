...
import com.clouditora.mq.common.command.BrokerUnregister;
...
public class DefaultRequestProcessor implements CommandRequestProcessor {
    ...
    public Command process(ChannelHandlerContext context, Command request) throws Exception {
        ...
        switch (requestCode) {
            ...
            default -> {
                log.error("{} request code [{}] is not supported", CoordinatorUtil.toAddress(context.channel()), request.getCode());
                return null;
            }
        }
    }
