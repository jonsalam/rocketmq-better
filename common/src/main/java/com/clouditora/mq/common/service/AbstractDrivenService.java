...
public abstract class AbstractDrivenService extends AbstractSimpleService {
    ...
    public void shutdown(boolean interrupt, long timeout) {
        ...
        if (this.waken.compareAndSet(false, true)) {
            ...
            this.resettableCountDownLatch.countDown();
        }
    ...
    protected void waitForWakeup() {
        ...
        try {
            boolean result = this.resettableCountDownLatch.await(this.interval, TimeUnit.MILLISECONDS);
            if (result) {
            ...
            onWakeup();
            this.resettableCountDownLatch.reset();
